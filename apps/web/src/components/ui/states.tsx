...
export function EmptyState({ title, message, description, action, icon, className }: { title?: string; /** @deprecated use title */ message?: string; description?: string; action?: React.ReactNode; icon?: React.ReactNode; className?: string }) {
  const heading = title ?? message ?? 'Өгөгдөл байхгүй';
  return (
    <div className={cn('flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border-default bg-bg-surface px-6 py-12 text-center', className)}>
      ...
      <p className="text-body-medium text-text-primary">{heading}</p>
...
export function LoadingState({ label = 'Ачааллаж байна…' }: { label?: string }) {
  return <p className="py-10 text-center text-body-sm text-text-muted" aria-busy>{label}</p>;
}
