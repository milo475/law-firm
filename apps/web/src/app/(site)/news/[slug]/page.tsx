...
import { formatDateLong, NEWS_CATEGORY_LABELS, readingMinutes } from '../news-utils';
...
export default async function PostPage({ params }: Params) {
  ...
  const categoryLabel = NEWS_CATEGORY_LABELS[post.category];
    ...
    <>
      ...
      <header className="bg-bg-page">
        <div className="mx-auto flex max-w-[848px] flex-col gap-3 px-5 py-8 md:gap-5 md:px-6 md:py-16">
          <Breadcrumb items={[{ label: 'Нүүр', href: '/' }, { label: 'Мэдээ', href: '/news' }, { label: post.title }]} className="[&_li:last-child>span]:line-clamp-1" />
          ...
          <div className="flex items-center gap-3 pt-1 md:gap-4 md:pt-2">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-caption text-text-muted md:text-body-sm md:font-normal">
                <span className="hidden md:inline">{ROLE_LABELS[post.author.role]} · {formatDateLong(post.publishedAt)}</span>
                <span className="md:hidden">{formatDate(post.publishedAt)}</span>
                {' · '}{minutes} минут унших
              </p>
            </div>
          </div>
      ...
      <article className="bg-bg-surface">
        <div className="mx-auto flex max-w-[848px] flex-col gap-5 px-5 py-10 md:gap-8 md:px-6 md:py-20">
          ...
          <div className="flex flex-wrap gap-2 md:gap-2.5 md:pt-4">
            <Link href={`/news?category=${post.category}`} className="focus-ring inline-flex items-center rounded-full border border-border-default bg-bg-page px-3 py-1.5 text-caption text-text-secondary transition-colors hover:border-border-strong hover:text-text-brand md:px-3.5 md:py-[7px]">
              {tag}
            </Link>
          </div>
      ...
      {related.length > 0 && (
        <section className="bg-bg-page" aria-labelledby="related-heading">
          <div className="mx-auto flex max-w-[1200px] flex-col gap-5 px-5 py-14 md:gap-8 md:px-6 md:py-24">
            <h2 id="related-heading" className="text-h3 md:text-h2">Холбоотой нийтлэл</h2>
            <div className="grid gap-5 md:grid-cols-3 md:gap-8">
              {related.map((p) => (
                <NewsCard key={p.id} overline={NEWS_CATEGORY_LABELS[p.category]} title={p.title} excerpt={p.excerpt} meta={`${formatDate(p.publishedAt)} · ${p.viewCount} үзсэн`} href={`/news/${p.slug}`} imageUrl={p.coverImageUrl} />
              ))}
            </div>
          </div>
        </section>
