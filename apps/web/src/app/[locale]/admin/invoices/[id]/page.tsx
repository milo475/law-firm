      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="flex flex-col gap-4 p-6">
          ...
          <dl className="flex flex-col gap-3">
            ...
            {inv.paidAt && <Row label="Төлсөн" value={formatDate(inv.paidAt, 'mn', true)} />}
          ...
          {awaiting ? (
            <>
              <div role="status" className="flex flex-col gap-1 rounded-md border-l-[3px] border-status-pending-fg bg-status-pending-bg px-4 py-3">
                ...
                <p className="text-body-sm text-text-secondary">{inv.paymentMarkedAt ? formatDate(inv.paymentMarkedAt, 'mn', true) : '—'}</p>
              </div>
              <dl className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1 rounded-md border-l-[3px] border-status-progress-fg bg-status-progress-bg px-4 py-3">
              ...
              <p className="text-body-sm text-text-secondary">
                {inv.paidAt ? formatDate(inv.paidAt, 'mn', true) : ''}
                {confirmer ? ` · ${confirmer}` : ''}
              </p>
            ...
            <div className="flex flex-col gap-1 rounded-md border-l-[3px] border-status-danger-fg bg-status-danger-bg px-4 py-3">
              ...
              <p className="text-caption text-text-muted">{formatDate(inv.paymentRejectedAt, 'mn', true)} · харилцагч дахин тэмдэглэхийг хүлээж байна</p>
            </div>
          ) : (
            <p className="text-body-sm text-text-secondary">
              {inv.status === 'DRAFT'
