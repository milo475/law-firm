      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="flex min-w-0 flex-col gap-2">
          ...
          <p className="text-body-sm text-text-secondary">
            {SERVICE_REQUEST_TYPE_LABELS[item.type]} · {CASE_TYPE_LABELS[item.caseType] ?? item.caseType} · {formatDate(item.createdAt, 'mn', true)}
          </p>
        </div>
        {open && (
...
function Decision({ item }: { item: ServiceRequestItem }) {
  const meta = [item.reviewedBy ? shortName(item.reviewedBy.firstName, item.reviewedBy.lastName) : null, item.reviewedAt ? formatDate(item.reviewedAt, 'mn', true) : null]
    .filter(Boolean)
    .join(' · ');
  switch (item.status) {
    case 'NEW':
