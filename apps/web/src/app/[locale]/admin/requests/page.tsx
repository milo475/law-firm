              ...
              <TableBody>
                {data.items.map((item) => (
                  ...
                  <TableRow key={item.id} className={cn(item.status === 'NEW' && 'bg-status-new-bg')}>
                    ...
                    <TableCell className="whitespace-nowrap">{formatDate(item.createdAt, 'mn', true)}</TableCell>
                ...
                {legacy.data.items.map((item) => (
                  <li key={item.id} className="flex flex-col gap-1 py-3">
                    ...
                    <p className="text-caption text-text-muted">
                      {item.name} · {item.phone}{item.email ? ` · ${item.email}` : ''} · {formatDate(item.createdAt, 'mn', true)}
                    </p>
                  </li>
                ))}
              </ul>
