...
export default function DocumentsPage() {
  ...
  const upload = useMutation({
    ...
    onSuccess: (doc, vars) => {
      toast.success(t('uploadedToast'), `${doc.name} хэрэгт нэмэгдлээ.`);
  ...
  function handleFiles(files: FileList | File[]) {
    ...
    if (!target) { toast.warning(t('pickCaseTitle'), t('pickCaseBody')); return; }
    for (const file of Array.from(files)) {
      if (file.size > MAX_BYTES) { toast.danger(t('tooLarge'), `${file.name} — 20MB-аас хэтэрсэн.`); continue; }
  ...
  async function openPreview(doc: DocumentItem) {
    try {
      ...
      toast.danger(t('previewFailed'), error instanceof ApiError ? error.message : t('tryAgain'));
    }
  }

  /** Desktop "name (lawyer)" / "name (you)"; mobile just the name, or "you". */
  function uploaderLabel(u: PublicUser, compact = false) {
    const name = shortName(u.firstName, u.lastName);
    if (u.id === user.id) return compact ? t('you') : t('uploaderYou', { name });
    if (compact) return name;
    return u.role === 'LAWYER' ? t('uploaderLawyer', { name }) : name;
  }

  const pickFile = () => fileInput.current?.click();
  const caseOptions = caseItems.map((c) => ({ value: c.id, label: `${c.caseNumber} · ${c.title}` }));
  const filterOptions = [{ value: 'ALL', label: t('all'), title: undefined as string | undefined }, ...caseItems.map((c) => ({ value: c.id, label: c.caseNumber, title: c.title }))];

  const actions = (d: DocumentItem) => (
    <div className="flex shrink-0 items-center gap-1.5">
      {previewable(d) ? (
        <IconAction label={`Урьдчилан харах: ${d.name}`} title={t('view')} onClick={() => void openPreview(d)}><EyeIcon /></IconAction>
      ) : (
        <span aria-hidden className="size-11" />
      )}
      <IconAction label={`Татах: ${d.name}`} title={t('download')} onClick={() => void download(d)}><DownloadIcon /></IconAction>
    </div>
  );
    ...
    <div className="flex flex-col gap-5 md:gap-4">
      ...
      <div className="hidden items-center justify-between gap-6 md:flex">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-body text-text-secondary">{anyLoading ? t('loading') : `Нийт ${allDocs.length} файл · ${formatBytes(totalSize)}`}</p>
        </div>
        <Button size="md" onClick={pickFile} disabled={upload.isPending || caseItems.length === 0}>{upload.isPending ? t('uploading') : t('addFile')}</Button>
      </div>

      {/* Upload (31:599 / 36:1193) — full-bleed white strip on mobile */}
      <section aria-label={t('uploadSection')} className="-mx-5 -mt-6 flex flex-col gap-4 bg-bg-surface p-5 md:mx-0 md:mt-0 md:bg-transparent md:p-0">
        <Select label={t('caseLabel')} placeholder={t('casePlaceholder')} options={caseOptions} value={caseId || undefined} onValueChange={setCaseId} disabled={cases.isLoading || caseItems.length === 0} wrapperClassName="md:max-w-[480px]" />
        <input ref={fileInput} type="file" multiple accept={ACCEPT} className="sr-only" aria-label={t('pickFile')} onChange={(e) => { if (e.target.files?.length) handleFiles(e.target.files); e.target.value = ''; }} />
        <div
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <p className="text-body-medium text-text-brand md:text-h4">
            <span className="md:hidden">{t('addFile')}</span>
            <span className="hidden md:inline">{t('dropzoneTitle')}</span>
          </p>
          <p className="text-caption text-text-secondary md:text-body-sm">
            <span className="md:hidden">{t('dropzoneHintShort')}</span>
            <span className="hidden md:inline">{t('dropzoneHint')}</span>
          </p>
          {upload.isPending && <p role="status" className="text-caption text-text-brand">{t('uploading')}</p>}
        </div>
      </section>
      ...
      <section aria-labelledby="all-files" className="flex flex-col gap-3.5 md:gap-4 md:pt-2">
        <div className="flex flex-col gap-3.5 md:flex-row md:items-center md:justify-between md:gap-4">
          <div className="flex items-center justify-between gap-3">
            <h3 id="all-files" className="font-serif text-[20px] font-semibold leading-7 md:text-h4">{t('allFiles')}</h3>
            {!anyLoading && <span className="text-caption text-text-muted md:hidden">{t('filesSummary', { count: allDocs.length, size: formatBytes(totalSize) })}</span>}
          </div>
          {caseItems.length > 1 && (
            <div role="radiogroup" aria-label={t('filterByCase')} className="flex flex-wrap gap-2.5">
        ...
        {cases.isError ? (
          <ErrorState message={cases.error instanceof ApiError ? cases.error.message : t('loadError')} onRetry={() => void cases.refetch()} />
        ) : anyLoading ? (
          <>
              ...
              <Table>
                <TableHead>
                  <TableRow className="h-12">
                    <TableHeaderCell className="h-12 w-[420px]">{t('columns.name')}</TableHeaderCell>
                    <TableHeaderCell className="h-12 w-[160px]">{t('columns.kind')}</TableHeaderCell>
                    <TableHeaderCell className="h-12 w-[220px]">{t('columns.uploadedBy')}</TableHeaderCell>
                    <TableHeaderCell className="h-12 w-[160px]">{t('columns.date')}</TableHeaderCell>
                    <TableHeaderCell className="h-12 w-[100px]">{t('columns.actions')}</TableHeaderCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                    ...
                    <Fragment key={caseItem.id}>
                      ...
                      {documents.map((d) => (
                        <TableRow key={d.id} interactive className="h-16">
                          ...
                          <TableCell className="h-16 whitespace-nowrap">{t(`kinds.${fileKind(d.mimeType, d.name)}`)}</TableCell>
                          <TableCell className="h-16 whitespace-nowrap">{uploaderLabel(d.uploadedBy)}</TableCell>
                          <TableCell className="h-16 whitespace-nowrap">{formatDate(d.createdAt, locale)}</TableCell>
                ...
                <div key={caseItem.id} className="flex flex-col gap-3.5">
                  ...
                  {documents.map((d) => (
                    <FileChip
                      ...
                      name={d.name}
                      mimeType={d.mimeType}
                      meta={`${formatBytes(d.size)} · ${uploaderLabel(d.uploadedBy, true)} · ${formatDate(d.createdAt, locale)}`}
            ...
            <DialogPrimitive.Content className="fixed left-1/2 top-1/2 z-50 flex h-[min(720px,calc(100vh-32px))] w-[calc(100vw-32px)] max-w-[880px] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-lg bg-bg-surface shadow-modal focus:outline-none">
                ...
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="flex min-w-0 flex-col gap-0.5">
                    ...
                    <DialogPrimitive.Description className="text-caption text-text-muted">
                      {formatBytes(preview.doc.size)} · {uploaderLabel(preview.doc.uploadedBy, true)} оруулсан · {formatDate(preview.doc.createdAt, locale)}
                    </DialogPrimitive.Description>
                  </div>
                </div>
                <DialogPrimitive.Close className="focus-ring inline-flex size-11 shrink-0 items-center justify-center rounded-md text-text-secondary hover:bg-bg-surface-alt" aria-label={t('close')}>
                  <CloseIcon size={44} />
              ...
              <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-border-default bg-bg-surface px-6 py-4">
                {(() => {
                  ...
                  return owner ? <p className="text-body-sm text-text-muted">{t('belongsTo', { caseNumber: owner.caseNumber })}</p> : <span />;
                })()}
                <div className="ml-auto flex gap-3">
                  <Button variant="ghost" size="md" onClick={() => setPreview(null)}>{t('close')}</Button>
                  <Button size="md" onClick={() => void download(preview.doc)}>{t('downloadCta')}</Button>
                </div>
              </div>
